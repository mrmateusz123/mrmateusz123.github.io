...
<body>
    ...
    <section id = "main">
        ...
        <?php
            $pol = mysqli_connect("localhost","root","","hodowla");
            $zap = "SELECT DISTINCT swinki.data_ur, swinki.miot, rasy.rasa FROM swinki JOIN rasy ON rasy.id = swinki.rasy_id WHERE rasy.id = 1;";
            $wyn = mysqli_query($pol,$zap);
            while($row = mysqli_fetch_array($wyn)){
        ...
        <?php
            $pol = mysqli_connect("localhost","root","","hodowla");
            $zap = "SELECT swinki.imie, swinki.cena, swinki.opis FROM swinki WHERE swinki.rasy_id = 1;";
            $wyn = mysqli_query($pol,$zap);
            while($row = mysqli_fetch_array($wyn)){
